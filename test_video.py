...
# Title: Test Video Loopback
# Author: jcochran
# Description: Simple test video to ensure input is encoded well to view it use $ mkfifo /tmp/test.ts     then $ ffplay /tmp/test.ts
...
class test_video(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "Test Video Loopback", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("Test Video Loopback")
        ...
        self.settings = Qt.QSettings("gnuradio/flowgraphs", "test_video")
    ...
    def set_frame_sync_cols(self, frame_sync_cols):
        ...
        self.qtgui_time_raster_sink_x_0.set_num_cols(self.frame_sync_cols)




def main(top_block_cls=test_video, options=None):
